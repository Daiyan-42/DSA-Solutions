...
public: 
	...
	int kruskalsWeight(vector<vector<int>> edges) 
	{ 
		vector<vector<int>> MSTedges;
    ...
    void criticalEdges() {
    	...
    	for (int i = 0; i < w; ++i)
    	{	
    		vector<int> edge = myedges[i];
    		myedges.erase(myedges.begin() + i);
    		int w = kruskalsWeight(myedges);
    		if (mstWeight < w) { cout << i << ","; }
    		myedges.insert(myedges.begin() + i, edge);
    	}
    	cout << "]" << endl; 	
    }

    void pseudoCritcalEdges() {
    	...
    	for (int i=0; i < myedges.size(); i++) {
    		...
    		for (auto mst: MSTs) {
    			for (auto path: mst) {
    				if (path[0] == myedges[i][0] && path[1] == myedges[i][1] && path[2] == myedges[i][2]) {
    					atleastone = true;
    					count += 1;
    				}
    			}
    		}
    		if (count < numofmst && atleastone) cout << i << ",";
    	}
    	cout << "]" << endl;
    }
};
